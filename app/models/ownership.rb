class Ownership < ApplicationRecord
  ...
  def display_name
    [([self.person.try(:full_name), self.person.try(:years)].select{ |t| t.present? }.join(' ')), self.institution.try(:display_name), self.religious_order.try(:order_name)].select{ |s| s.present? }.join(', ')
  end

  def full_display_name
    text = self.display_name
    text += "; " if self.display_name.present? && self.location.present?
    text += self.location.try(:city_region_country) if self.location.present?
    text += ". " if self.display_name.present? || self.location.present?
