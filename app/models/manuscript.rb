class Manuscript < ApplicationRecord
  ...
  def display_name
    d_name = [self.institution.try(:location).present? ? self.institution.location.city : self.genesis_location.city, self.institution.try(:name_orig), self.shelfmark].select{ |d| d.present? }.join(",")
