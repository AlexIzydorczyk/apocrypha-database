class ModernSource < ApplicationRecord
  ...
  def display_name
    ...
    # publication title
    s += title(self.publication_title_language, self.publication_title_orig, self.publication_title_transliteration, self.publication_title_transliteration) + ". "
    ...
    if self.part_no.present? || self.part_title_orig.present? || self.part_title_transliteration.present? || self.part_title_translation.present?
      ...
      s += [
        self.part_no,
        title(self.part_title_language, self.part_title_orig, self.part_title_transliteration, self.part_title_translation)
      ].select{ |v| v.present? }.join(", ")
    end
    ...
    #first url
    if self.source_urls.count > 0
      s += "Accessed " + (self.source_urls.first.date_accessed.strftime("%-d %B, %Y")) + ". " if self.source_urls.first.date_accessed.present?
      s += self.source_urls.first.url + ". "
    end

    #doi
    s += "doi: " + self.DOI + ". " if self.DOI.present?

    s.strip.gsub("  ", " ").gsub(" .", ".").gsub("...", ".").gsub("..", ".")
  end
  ...
  def title language, orig, translit, transla, show_quotes=false
    s = ""
    title = language.present? && language.requires_transliteration ? translit : orig
    if title.present?
      title = '"' + title + '"' if show_quotes
      s += title + " "
    end
