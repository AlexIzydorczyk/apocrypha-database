class ModernSource < ApplicationRecord
  ...
  has_many :author_references, -> { author.order(:sequence_no) }, as: :record, class_name: "PersonReference"
  ...
  def set_display_name
    ...
    # title
    (s += title(self.title_language, self.title_orig, self.title_transliteration, self.title_transliteration, true, false) + ". ") if ['book_chapter', 'journal_article', 'handwritten_document', 'unpublished_document', 'web_page'].include?(self.source_type)
  ...
  def person_list people, are_editors=false, first_list=false 
    names = people.order(:sequence_no, last_name_vernacular: :asc).map.with_index{ |p, i| p.modern_source_display(i>0 || !first_list) }
